...
abstract class Entity(initialX:Int, initialY:Int, val elements:List<Element>): Element(initialX, initialY) {
    ...
    private fun findDistances(target:Point) {
        val initial = elements.find { it.x == target.x && it.y == target.y}
        if (initial == null || !(initial is Open)) {
            println("<< target is not open >>")
            return
        }
        val adjacent = getAdjacent(initial as Open)
    }

    private fun getAdjacent(p:Open):List<Open> {
        val list = mutableListOf<Open>()
        val left = elements.find { it.x == p.x +1 && it.y == p.y}
        if (left != null && left is Open) {
            list.add(left)
        }
        return list
    }
